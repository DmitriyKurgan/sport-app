...
The repo includes [`render.yaml`](render.yaml) — a Blueprint that provisions both the database and the web service in one click.

1. Render Dashboard → **New +** → **Blueprint**.
2. Connect your GitHub account and pick this repo.
3. Render reads `render.yaml` and shows **fittrack-api** + **fittrack-db**. Click **Apply**.
4. Wait ~5 min for the first build (Docker multi-stage).
5. Once green, copy the public URL (looks like `https://fittrack-api.onrender.com`).

What `render.yaml` does automatically:

- Provisions a free PostgreSQL 15 database `fittrack-db`.
- Builds the API image from `fittrack-api/Dockerfile`.
- Wires `DATABASE_HOST/PORT/USER/PASSWORD/NAME` from the database into the service env.
